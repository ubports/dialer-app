...
Dialog {
    ...
    function runDeleteOperation(afterMonth) {
        ...
        removeFilter.filterValue = Qt.formatDateTime(d, "yyyy-MM-ddTHH:mm:ss.zzz")
        deleteEventModel.removeAll();
    }
    ...
    states: [
        ...
        State {
            name: "PENDING_DELETE"
            PropertyChanges { target: dialog; text: i18n.tr("Deleting %1 records...").arg(deleteEventModel.count) }
        ...
        State {
            name: "FINISHED_WITH_TIMEOUT"
            PropertyChanges { target: dialog; text: i18n.tr("Removed %1 records, something went wrong").arg(deleteEventModel.deletedCount) }
            PropertyChanges { target: dismissBtn; visible: true }
        }
    ]
    ...
    HistoryManager {
        ...
        onOperationEnded: {
            // timeout ?
            if (count > 0 && count > deletedCount) {
                dialog.state = "FINISHED_WITH_TIMEOUT"
            } else {
                if (count === 0) {
